...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        # 構建網格點
        self.grid = self.build_grid(grid_range, grid_size, spline_order).to(dtype)
    ...
    def build_grid(self, grid_range, grid_size, spline_order):
        ...
        grid = (
            (
                torch.arange(
                    -spline_order, grid_size + spline_order + 1, dtype=self.dtype
                )
                * h
                + grid_range[0]
            )
    ...
    def initialize_weights(
        ...
        base_weight = torch.nn.Parameter(
            torch.Tensor(out_features, in_features).to(self.dtype)
        )
        spline_weight = torch.nn.Parameter(
            torch.Tensor(out_features, in_features, grid_size + spline_order).to(
                self.dtype
            )
        )
