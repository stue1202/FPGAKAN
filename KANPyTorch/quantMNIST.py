...
def save_layer_weights_to_txt(model, layer_index, base_dir, num_bits):
    ...
    spline_weight_file = os.path.join(
        folder_name, f"layer{layer_index}_spline_weight.txt"
    )
    spline_scaler_file = os.path.join(
        folder_name, f"layer{layer_index}_spline_scaler.txt"
    )

    with open(base_weight_file, "w") as f:
        base_weight_data = layer.base_weight.detach().cpu().numpy()
        quantized_base_weight, scale, zero_point = quantize_tensor(
            torch.tensor(base_weight_data), num_bits
        )
    ...
    with open(spline_weight_file, "w") as f:
        ...
        quantized_spline_weight, scale, zero_point = quantize_tensor(
            torch.tensor(spline_weight_data), num_bits
        )
        f.write(f"scale: {scale}\n")
        f.write(f"zero_point: {zero_point}\n")
    ...
    if layer.spline_scaler is not None:
        with open(spline_scaler_file, "w") as f:
            ...
            quantized_spline_scaler, scale, zero_point = quantize_tensor(
                torch.tensor(spline_scaler_data), num_bits
            )
            f.write(f"scale: {scale}\n")
            f.write(f"zero_point: {zero_point}\n")
...
def load_quantized_weights_from_txt(model, layer_index, base_dir, num_bits):
    ...
    layer.base_weight.data = torch.tensor(
        (quantized_base_weight - zero_point) * scale, dtype=torch.float32
    ).view_as(layer.base_weight)

    quantized_spline_weight, scale, zero_point = read_quantized_file(spline_weight_file)
    layer.spline_weight.data = torch.tensor(
        (quantized_spline_weight - zero_point) * scale, dtype=torch.float32
    ).view_as(layer.spline_weight)

    if os.path.exists(spline_scaler_file):
        quantized_spline_scaler, scale, zero_point = read_quantized_file(
            spline_scaler_file
        )
        layer.spline_scaler.data = torch.tensor(
            (quantized_spline_scaler - zero_point) * scale, dtype=torch.float32
        ).view_as(layer.spline_scaler)
